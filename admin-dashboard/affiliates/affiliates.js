window.AXIOM_ADMIN_AFFILIATES = {
  ...
  async loadAffiliates() {
    ...
    try {
      ...
      if (this.tableBody) {
        this.tableBody.innerHTML = `<tr><td colspan="10">Failed to load affiliates.</td></tr>`;
      }
    }
  },
  ...
  applyFilters() {
    ...
    this.filteredAffiliates = this.affiliates.filter((item) => {
      const matchesStatus = status === "all" ? true : String(item.status || "").toLowerCase() === status;
  ...
  renderStats() {
    ...
    const claimable = this.affiliates.reduce((sum, item) => sum + Number(item.claimable_commission || 0), 0);
  ...
  renderAffiliateDetail(summary, detailData) {
    ...
    this.setText("affiliateDetailClicks", String(summary?.total_clicks || 0));
    this.setText("affiliateDetailConversions", String(summary?.total_conversions || 0));
...
document.addEventListener("DOMContentLoaded", function () {
  if (window.AXIOM_ADMIN_AFFILIATES) {
    window.AXIOM_ADMIN_AFFILIATES.init();
  }
});
